...
export default function App() {
  ...
  const savePriceDB   = async (v) => { setPriceDB(v); await supabase.from('price_db').delete().neq('storeId','___never___').gte('id',1); if(v.length>0){ const clean=v.map(p=>({product:p.product,format:p.format,brand:p.brand||'',storeId:p.storeId||'',store_name:'',price:parseFloat(p.price),date:p.date||new Date().toISOString()})); await supabase.from('price_db').insert(clean); } };
